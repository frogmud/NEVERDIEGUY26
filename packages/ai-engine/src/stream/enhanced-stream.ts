...
export interface EnhancedStreamConfig extends StreamConfig {
  ...
  /** Enable NPC reactions to entries */
  enableReactions: boolean;
  /** Chance of each NPC reacting to an entry */
...
export function generateEnhancedDayStream(
  ...
  if (config.enableReactions) {
    return state.entries.map(entry => ({
      ...entry,
      reactions: generateReactionsForEntry(entry, activeNPCs, rng, config.reactionChance),
    }));
  }
...
/**
 * Generate reactions for a stream entry
 */
function generateReactionsForEntry(
  entry: StreamEntry,
  activeNPCs: string[],
  rng: SeededRng,
  reactionChance: number
  ...
  for (const npcSlug of otherNPCs) {
    ...
    const relationshipType = categorizeRelationship(reactor, entry.speakerSlug);
