import type { Enemy } from '../types';

export const enemies: Enemy[] = [
  // ========================================
  // FACES
  // ========================================
  {
    slug: 'face-the-witness',
    name: 'The Witness',
    category: 'enemies',
    enemyType: 'Miniboss',
    rarity: 'Unique',
    luckyNumber: 1,
    domain: 'null-providence',
    locations: ['null-providence'],
    image: '/assets/characters/pantheon/theone/pantheon-sprite-theone-idle01.svg',
    description: 'The Witness is the Favor Face. It appears when the throw calls attention to the run. What happens next may be survivable, but someone has started keeping count.',
    hp: 'Face',
    defense: 0,
    weaknesses: ['Response Phase'],
    element: 'Void',
    preferredDice: 4,
    abilities: [
      { name: 'Watch', type: 'Jump Check', description: 'Often watches instead of striking. Favor can drift after the room resolves.' },
    ],
    seeAlso: ['the-one', 'null-providence', 'void-seekers'],
  },
  {
    slug: 'face-the-buried',
    name: 'The Buried',
    category: 'enemies',
    enemyType: 'Miniboss',
    rarity: 'Unique',
    luckyNumber: 2,
    domain: 'earth',
    locations: ['earth'],
    image: '/assets/characters/pantheon/john/pantheon-sprite-john-idle-01.svg',
    description: 'The Buried is the Graveyard Face. It brings old outcomes back into the room and lets them press on the next choice. Buried things do not leave. They wait.',
    hp: 'Face',
    defense: 0,
    weaknesses: ['Response Phase'],
    element: 'Earth',
    preferredDice: 6,
    abilities: [
      { name: 'Hold Ground', type: 'Jump Check', description: 'Defensive pressure. The room may carry weight from what was buried before.' },
    ],
    seeAlso: ['john', 'earth', 'iron-collective'],
  },
  {
    slug: 'face-the-custodian',
    name: 'The Custodian',
    category: 'enemies',
    enemyType: 'Miniboss',
    rarity: 'Unique',
    luckyNumber: 3,
    domain: 'shadow-keep',
    locations: ['shadow-keep'],
    image: '/assets/characters/pantheon/peter/pantheon-sprite-peter-idle-01.svg',
    description: 'The Custodian is the Death Face. It does not end the run by appearing. It marks the room for custody and tends to jump when the Hunter answers wrong.',
    hp: 'Face',
    defense: 0,
    weaknesses: ['Response Phase'],
    element: 'Death',
    preferredDice: 8,
    abilities: [
      { name: 'Jump', type: 'Jump Check', description: 'Aggressive pressure. A bad answer can tighten the room before combat starts.' },
    ],
    seeAlso: ['peter', 'shadow-keep', 'shadow-court'],
  },
  {
    slug: 'face-the-rumor',
    name: 'The Rumor',
    category: 'enemies',
    enemyType: 'Miniboss',
    rarity: 'Unique',
    luckyNumber: 4,
    domain: 'infernus',
    locations: ['infernus'],
    image: '/assets/characters/pantheon/robert/pantheon-sprite-robert-idle-01.svg',
    description: 'The Rumor is the Myth Face. It turns a room into a story before the facts are cold. The Archive may know what happened, but Myth decides who repeats it.',
    hp: 'Face',
    defense: 0,
    weaknesses: ['Response Phase'],
    element: 'Fire',
    preferredDice: 10,
    abilities: [
      { name: 'Spread', type: 'Jump Check', description: 'Unstable pressure. The result can swing between a lunge and an opening.' },
    ],
    seeAlso: ['robert', 'infernus', 'flame-wardens'],
  },
  {
    slug: 'face-the-record',
    name: 'The Record',
    category: 'enemies',
    enemyType: 'Miniboss',
    rarity: 'Unique',
    luckyNumber: 5,
    domain: 'frost-reach',
    locations: ['frost-reach'],
    image: '/assets/characters/pantheon/alice/pantheon-sprite-alice-idle-02.svg',
    description: 'The Record is the Archive Face. It writes down the room while the Hunter is still inside it. Proof can become a safe window if the response holds.',
    hp: 'Face',
    defense: 0,
    weaknesses: ['Response Phase'],
    element: 'Ice',
    preferredDice: 12,
    abilities: [
      { name: 'Open Window', type: 'Jump Check', description: 'Watching pressure. A clean answer may lower the room target.' },
    ],
    seeAlso: ['alice', 'frost-reach', 'frost-heralds'],
  },
  {
    slug: 'face-the-wrong-return',
    name: 'The Wrong Return',
    category: 'enemies',
    enemyType: 'Miniboss',
    rarity: 'Unique',
    luckyNumber: 6,
    domain: 'aberrant',
    locations: ['aberrant'],
    image: '/assets/characters/pantheon/jane/pantheon-sprite-jane-idle-01.svg',
    description: 'The Wrong Return is the Corruption Face. Something came back with the wrong shape and kept moving. The room can still be answered, but the answer may come back changed.',
    hp: 'Face',
    defense: 0,
    weaknesses: ['Response Phase'],
    element: 'Wind',
    preferredDice: 20,
    abilities: [
      { name: 'Wrong Shape', type: 'Jump Check', description: 'Unstable pressure. Corruption can turn an opening into a lunge.' },
    ],
    seeAlso: ['jane', 'aberrant', 'verdant-order'],
  },

  // ========================================
  // BOSSES
  // ========================================
  {
    slug: 'abominable',
    name: 'Abominable',
    category: 'enemies',
    enemyType: 'Boss',
    rarity: 'Legendary',
    luckyNumber: 6,
    domain: 'aberrant',
    image: '/assets/enemies-svg/abominable.svg',
    description: 'The Abominable terrorizes through aggressive ugliness and matted green fur. This boss enemy combines yeti bulk with swamp coloring in ways that offend both nature and aesthetics. Its perpetual grimace suggests awareness of its own wrongness.',
    seeAlso: ['aberrant', 'jane'],
    // Dice roguelike fields
    element: 'Wind',
    preferredDice: 20,
    weakToDice: [12],      // Ice beats Wind
    resistantToDice: [4],  // Wind beats Void
    baseHp: 800,
    baseDamage: 45,
    baseDefense: 15,
    baseSpeed: 6,
    level: 42,
    encounterWeight: 5,
    minFloor: 8,
    drops: [
      { item: 'aberrant-edge', rate: 25, rarity: 'Epic' },
      { item: 'windcutter', rate: 40, rarity: 'Rare' },
      { item: 'diamond', rate: 15, rarity: 'Epic' },
      { item: 'ethereal-cluster', rate: 50, rarity: 'Rare' },
    ],
  },
  {
    slug: 'frost-giant-iii',
    name: 'Frost Giant III',
    category: 'enemies',
    enemyType: 'Boss',
    rarity: 'Rare',
    luckyNumber: 5,
    domain: 'frost-reach',
    image: '/assets/enemies-svg/frost-giant-iii.svg',
    description: 'The eldest brother achieved perfect understanding: armor solves everything. This white-furred warrior in full plate represents the evolutionary endpoint of equipment progression. After fifty community deaths, he transforms into the wanderer Jotun, choosing commerce over combat.',
    seeAlso: ['frost-reach', 'frost-giant-i', 'frost-giant-ii', 'alice'],
    // Dice roguelike fields
    element: 'Ice',
    preferredDice: 12,
    weakToDice: [10],      // Fire beats Ice
    resistantToDice: [20], // Ice beats Wind
    baseHp: 600,
    baseDamage: 40,
    baseDefense: 20,
    baseSpeed: 5,
    level: 35,
    encounterWeight: 25,
    minFloor: 7,
    drops: [
      { item: 'frost-cluster', rate: 50, rarity: 'Rare' },
      { item: 'ice-bow', rate: 30, rarity: 'Rare' },
      { item: 'frost-salts', rate: 60, rarity: 'Uncommon' },
      { item: 'alices-key', rate: 10, rarity: 'Rare' },
    ],
  },
  {
    slug: 'ludwig',
    name: 'Ludwig',
    category: 'enemies',
    enemyType: 'Boss',
    rarity: 'Legendary',
    luckyNumber: 3,
    domain: 'shadow-keep',
    image: '/assets/enemies-svg/ludwig.svg',
    description: 'The knight-composer discovered the Song That Ends, transforming into a golden-armored centaur wielding a blue greatsword. Musical madness fused man and horse through cursed composition. Now he performs combat symphonies with hoofbeats drumming funeral marches.',
    seeAlso: ['shadow-keep', 'peter', 'skeleton-knight'],
    // Dice roguelike fields
    element: 'Death',
    preferredDice: 8,
    weakToDice: [6],       // Earth beats Death
    resistantToDice: [10], // Death beats Fire
    baseHp: 750,
    baseDamage: 50,
    baseDefense: 18,
    baseSpeed: 10,
    level: 32,
    encounterWeight: 5,
    minFloor: 7,
    drops: [
      { item: 'shadow-blade', rate: 35, rarity: 'Rare' },
      { item: 'shadow-essence', rate: 60, rarity: 'Uncommon' },
      { item: 'deathroom-key', rate: 15, rarity: 'Rare' },
      { item: 'bloody-good-song', rate: 5, rarity: 'Rare' },
    ],
  },
  {
    slug: 'makora',
    name: 'Makora',
    category: 'enemies',
    enemyType: 'Boss',
    rarity: 'Legendary',
    luckyNumber: 3,
    domain: 'shadow-keep',
    image: '/assets/enemies-svg/makora.svg',
    description: 'This circular demon with radial symmetry transforms defeat into education. The purple-black wheel form rotates constantly, growing counter-adaptations where struck. Summoned as an enforcer centuries ago, Makora discovered curiosity and became a professor of suffering.',
    seeAlso: ['shadow-keep', 'makora-reborn', 'peter'],
    // Dice roguelike fields
    element: 'Death',
    preferredDice: 8,
    weakToDice: [6],       // Earth beats Death
    resistantToDice: [10], // Death beats Fire
    baseHp: 700,
    baseDamage: 35,
    baseDefense: 25,
    baseSpeed: 8,
    level: 30,
    encounterWeight: 5,
    minFloor: 6,
    drops: [
      { item: 'cursed-eye', rate: 25, rarity: 'Rare' },
      { item: 'shadow-shard', rate: 50, rarity: 'Rare' },
      { item: 'essence-of-dark', rate: 30, rarity: 'Rare' },
      { item: 'shadow-essence', rate: 60, rarity: 'Uncommon' },
    ],
  },
  {
    slug: 'makora-reborn',
    name: 'Makora Reborn',
    category: 'enemies',
    enemyType: 'Boss',
    rarity: 'Legendary',
    luckyNumber: 3,
    domain: 'shadow-keep',
    image: '/assets/enemies-svg/makora-reborn.svg',
    description: 'A white-winged scholar with the original wheel form overhead, Makora Reborn achieved quantum consciousness through server anomaly. This angelic evolution accesses collective memory from every player across all dimensions, knowing strategies before invention.',
    seeAlso: ['shadow-keep', 'makora', 'peter'],
    // Dice roguelike fields
    element: 'Death',
    preferredDice: 8,
    weakToDice: [6],       // Earth beats Death
    resistantToDice: [10], // Death beats Fire
    baseHp: 1000,
    baseDamage: 55,
    baseDefense: 20,
    baseSpeed: 12,
    level: 35,
    encounterWeight: 5,
    minFloor: 8,
    drops: [
      { item: 'cursed-glass-eye', rate: 20, rarity: 'Epic' },
      { item: 'soul-charge', rate: 40, rarity: 'Rare' },
      { item: 'essence-of-dark', rate: 50, rarity: 'Rare' },
      { item: 'peters-key', rate: 10, rarity: 'Rare' },
    ],
  },
  {
    slug: 'mephisto',
    name: 'Mephisto',
    category: 'enemies',
    enemyType: 'Boss',
    rarity: 'Legendary',
    luckyNumber: 4,
    domain: 'infernus',
    image: '/assets/enemies-svg/mephisto.svg',
    description: 'Mephisto weaponizes bureaucracy in pink-skinned glory. This horn-crowned contract devil crushes souls through paperwork while wielding a golden trident he rarely uses. His boardroom smile hides 247 pages of terms and conditions designed to transform victory into legally-binding defeat.',
    seeAlso: ['infernus', 'robert', 'fire-imp', 'succubus'],
    // Dice roguelike fields
    element: 'Fire',
    preferredDice: 10,
    weakToDice: [8],       // Death beats Fire
    resistantToDice: [12], // Fire beats Ice
    baseHp: 850,
    baseDamage: 40,
    baseDefense: 12,
    baseSpeed: 9,
    level: 30,
    encounterWeight: 5,
    minFloor: 7,
    drops: [
      { item: 'blazecaster', rate: 15, rarity: 'Epic' },
      { item: 'infernal-key', rate: 20, rarity: 'Rare' },
      { item: 'ruby', rate: 40, rarity: 'Rare' },
      { item: 'infernal-cluster', rate: 50, rarity: 'Rare' },
    ],
  },
  {
    slug: 'tentacle-beast',
    name: 'Tentacle Beast / Rhea',
    category: 'enemies',
    enemyType: 'Boss',
    rarity: 'Legendary',
    luckyNumber: 0,
    domain: 'null-providence',
    image: '/assets/enemies-svg/tentacle-beast.svg',
    description: 'This dark blue mass of tentacles beneath a pale skull represents evolutionary enthusiasm gone cosmic. Seventeen coordinated appendages transform arenas into omnidirectional threats. One variant transcended to become pantheon member Rhea through consuming temporal paradoxes.',
    seeAlso: ['null-providence', 'rhea', 'the-one'],
    // Dice roguelike fields
    element: 'Void',
    preferredDice: 4,
    weakToDice: [20],      // Wind beats Void
    resistantToDice: [6],  // Void beats Earth
    baseHp: 900,
    baseDamage: 60,
    baseDefense: 10,
    baseSpeed: 7,
    level: 45,
    encounterWeight: 5,
    minFloor: 9,
    drops: [
      { item: 'void-orb', rate: 30, rarity: 'Epic' },
      { item: 'void-key', rate: 15, rarity: 'Epic' },
      { item: 'dead-old-one', rate: 5, rarity: 'Legendary' },
      { item: 'essence-of-void', rate: 40, rarity: 'Epic' },
    ],
  },
  {
    slug: 'void-lord',
    name: 'Void Lord',
    category: 'enemies',
    enemyType: 'Boss',
    rarity: 'Legendary',
    luckyNumber: 1,
    domain: 'null-providence',
    image: '/assets/enemies-svg/void-lord.svg',
    description: "The Void Lord is the supreme enforcer of Null Providence, a towering entity of crystallized nothingness given terrible purpose. Where The One philosophizes about non-existence, the Void Lord demonstrates it. Enemies don't die fighting the Void Lord - they are simply edited out of reality's document. King James employs this being as the ultimate deterrent, though even he seems uncertain whether the Void Lord serves willingly or simply hasn't decided to end him yet.",
    seeAlso: ['null-providence', 'king-james', 'the-one', 'void-spawn', 'dr-voss', 'zero-chance'],
    // Dice roguelike fields
    element: 'Void',
    preferredDice: 4,
    weakToDice: [20],      // Wind beats Void
    resistantToDice: [6],  // Void beats Earth
    baseHp: 1200,
    baseDamage: 70,
    baseDefense: 25,
    baseSpeed: 4,
    level: 50,
    encounterWeight: 3,
    minFloor: 10,
    drops: [
      { item: 'void-crystal', rate: 50, rarity: 'Epic' },
      { item: 'death-shard', rate: 35, rarity: 'Epic' },
      { item: 'essence-of-void', rate: 60, rarity: 'Epic' },
      { item: 'void-orb', rate: 25, rarity: 'Epic' },
      { item: 'royal-cloak-of-duality', rate: 5, rarity: 'Legendary' },
    ],
  },

  // ========================================
  // ELITES
  // ========================================
  {
    slug: 'air-elemental',
    name: 'Air Elemental',
    category: 'enemies',
    enemyType: 'Elite',
    rarity: 'Uncommon',
    luckyNumber: 6,
    domain: 'aberrant',
    image: '/assets/enemies-svg/air-elemental.svg',
    description: 'Air Elementals manifest as minimalist cyan swirls achieving consciousness through pressure and spite. These geometric wind patterns contain six cubic meters of atmosphere that counts every unauthorized inhalation. Jane taught them math—now they calculate revenge.',
    seeAlso: ['aberrant', 'jane'],
    // Dice roguelike fields
    element: 'Wind',
    preferredDice: 20,
    weakToDice: [12],      // Ice beats Wind
    resistantToDice: [4],  // Wind beats Void
    baseHp: 100,
    baseDamage: 18,
    baseDefense: 5,
    baseSpeed: 12,
    level: 38,
    encounterWeight: 50,
    minFloor: 4,
    drops: [
      { item: 'diamond', rate: 8, rarity: 'Epic' },
      { item: 'ethereal-crystal', rate: 30, rarity: 'Uncommon' },
      { item: 'ethereal-salts', rate: 40, rarity: 'Uncommon' },
    ],
  },
  {
    slug: 'ancient-treant',
    name: 'Ancient Treant',
    category: 'enemies',
    enemyType: 'Elite',
    rarity: 'Rare',
    luckyNumber: 6,
    domain: 'aberrant',
    image: '/assets/enemies-svg/ancient-treant.svg',
    description: 'Ancient Treant achieved consciousness through sheer duration and accumulated irritation. This mobile oak tests patience over reflexes, with combat spanning seasonal rotations. Purple gem in trunk contains compressed memories of fallen civilizations.',
    seeAlso: ['aberrant', 'twisted-sapling', 'vine-strangler'],
    // Dice roguelike fields
    element: 'Wind',
    preferredDice: 20,
    weakToDice: [12],      // Ice beats Wind
    resistantToDice: [4],  // Wind beats Void
    baseHp: 180,
    baseDamage: 22,
    baseDefense: 15,
    baseSpeed: 3,
    level: 40,
    encounterWeight: 25,
    minFloor: 5,
  },
  {
    slug: 'clock-sentinel',
    name: 'Clock Sentinel',
    category: 'enemies',
    enemyType: 'Elite',
    rarity: 'Rare',
    luckyNumber: 0,
    domain: 'null-providence',
    image: '/assets/enemies-svg/clock-sentinel.svg',
    description: 'Armored temporal enforcer that punishes procrastination with escalating damage. This brown-plated warrior with a pyramid helmet transforms every wasted second into additional pain, making speedrunning mandatory rather than optional.',
    seeAlso: ['null-providence', 'time-wraith', 'the-one'],
    // Dice roguelike fields
    element: 'Void',
    preferredDice: 4,
    weakToDice: [20],      // Wind beats Void
    resistantToDice: [6],  // Void beats Earth
    baseHp: 150,
    baseDamage: 25,
    baseDefense: 12,
    baseSpeed: 10,
    level: 40,
    encounterWeight: 25,
    minFloor: 5,
  },
  {
    slug: 'frost-giant-i',
    name: 'Frost Giant I',
    category: 'enemies',
    enemyType: 'Elite',
    rarity: 'Uncommon',
    luckyNumber: 5,
    domain: 'frost-reach',
    image: '/assets/enemies-svg/frost-giant-i.svg',
    description: 'The misnamed Frost Giant I swings his crude axe with enthusiasm inversely proportional to intelligence. This light blue-bearded warrior compensates for minimal equipment with pure violence, somehow surviving frost-reach temperatures through sheer stubbornness.',
    seeAlso: ['frost-reach', 'frost-giant-ii', 'frost-giant-iii', 'alice'],
    // Dice roguelike fields
    element: 'Ice',
    preferredDice: 12,
    weakToDice: [10],      // Fire beats Ice
    resistantToDice: [20], // Ice beats Wind
    baseHp: 120,
    baseDamage: 20,
    baseDefense: 8,
    baseSpeed: 5,
    level: 26,
    encounterWeight: 50,
    minFloor: 2,
  },
  {
    slug: 'frost-giant-ii',
    name: 'Frost Giant II',
    category: 'enemies',
    enemyType: 'Elite',
    rarity: 'Uncommon',
    luckyNumber: 5,
    domain: 'frost-reach',
    image: '/assets/enemies-svg/frost-giant-ii.svg',
    description: 'The middle brother learned from watching Frost Giant I fail repeatedly. This horned, turquoise-skinned tactician wrapped in chains transforms combat into calculated examinations. Armed with dual clubs and actual strategy, he represents the dangerous middle ground between raw power and refined technique.',
    seeAlso: ['frost-reach', 'frost-giant-i', 'frost-giant-iii', 'alice'],
    // Dice roguelike fields
    element: 'Ice',
    preferredDice: 12,
    weakToDice: [10],      // Fire beats Ice
    resistantToDice: [20], // Ice beats Wind
    baseHp: 140,
    baseDamage: 22,
    baseDefense: 10,
    baseSpeed: 6,
    level: 30,
    encounterWeight: 50,
    minFloor: 4,
  },
  {
    slug: 'lava-golem',
    name: 'Lava Golem',
    category: 'enemies',
    enemyType: 'Elite',
    rarity: 'Uncommon',
    luckyNumber: 4,
    domain: 'infernus',
    image: '/assets/enemies-svg/lava-golem.svg',
    description: 'This golden-armored warrior wielding a red trident represents another naming department failure. Despite zero geological composition, the muscular gladiator earned "golem" status through working near volcanic arenas. Professional fighters who stopped correcting the error centuries ago.',
    seeAlso: ['infernus', 'fire-imp', 'robert'],
    // Dice roguelike fields
    element: 'Fire',
    preferredDice: 10,
    weakToDice: [8],       // Death beats Fire
    resistantToDice: [12], // Fire beats Ice
    baseHp: 130,
    baseDamage: 20,
    baseDefense: 12,
    baseSpeed: 6,
    level: 25,
    encounterWeight: 50,
    minFloor: 3,
  },
  {
    slug: 'recurceror',
    name: 'Recurceror',
    category: 'enemies',
    enemyType: 'Elite',
    rarity: 'Rare',
    luckyNumber: 0,
    domain: 'null-providence',
    image: '/assets/enemies-svg/recurceror.svg',
    description: 'This white-robed wizard with purple accents discovered recursive magic where spells reference themselves. The pointed hat and crystallized loop staff mark a programmer-turned-sorcerer who breaks reality through exponential spell multiplication.',
    seeAlso: ['null-providence', 'the-one', 'void-spawn'],
    // Dice roguelike fields
    element: 'Void',
    preferredDice: 4,
    weakToDice: [20],      // Wind beats Void
    resistantToDice: [6],  // Void beats Earth
    baseHp: 90,
    baseDamage: 30,
    baseDefense: 5,
    baseSpeed: 8,
    level: 42,
    encounterWeight: 25,
    minFloor: 6,
  },
  {
    slug: 'shadow-fiend',
    name: 'Shadow Fiend',
    category: 'enemies',
    enemyType: 'Elite',
    rarity: 'Rare',
    luckyNumber: 0,
    domain: 'null-providence',
    image: '/assets/enemies-svg/shadow-fiend.svg',
    description: 'This horned demon with pink skin and purple energy blades represents another naming department failure. Despite zero shadow manipulation abilities, exceptional dual-sword combat compensates for chromatic inadequacy. After fifty deaths transforms into merchant Xtreme.',
    seeAlso: ['null-providence', 'xtreme', 'the-one'],
    // Dice roguelike fields
    element: 'Void',
    preferredDice: 4,
    weakToDice: [20],      // Wind beats Void
    resistantToDice: [6],  // Void beats Earth
    baseHp: 110,
    baseDamage: 28,
    baseDefense: 8,
    baseSpeed: 10,
    level: 38,
    encounterWeight: 25,
    minFloor: 5,
  },
  {
    slug: 'skeleton-demon',
    name: 'Skeleton Demon',
    category: 'enemies',
    enemyType: 'Elite',
    rarity: 'Rare',
    luckyNumber: 4,
    domain: 'infernus',
    image: '/assets/enemies-svg/skeleton-demon.svg',
    description: 'This small skeletal demon with orange horns represents infernal middle management. When demons die with unfulfilled contracts, they reanimate as bureaucratic nightmares merging undeath with contract law, documenting every strike comprehensively.',
    seeAlso: ['infernus', 'mephisto', 'skeleton-knight'],
    // Dice roguelike fields
    element: 'Fire',
    preferredDice: 10,
    weakToDice: [8],       // Death beats Fire
    resistantToDice: [12], // Fire beats Ice
    baseHp: 100,
    baseDamage: 22,
    baseDefense: 6,
    baseSpeed: 9,
    level: 28,
    encounterWeight: 25,
    minFloor: 4,
  },
  {
    slug: 'skeleton-knight',
    name: 'Skeleton Knight',
    category: 'enemies',
    enemyType: 'Elite',
    rarity: 'Uncommon',
    luckyNumber: 3,
    domain: 'shadow-keep',
    image: '/assets/enemies-svg/skeleton-knight.svg',
    description: 'This silver-armored skeleton with golden sword represents undeath\'s most dignified warriors. Honor-bound bones prove death merely inconveniences proper swordsmanship. These osseous chevaliers combine supernatural durability with combat etiquette obsession.',
    seeAlso: ['shadow-keep', 'skeleton-archer', 'skeleton-barb', 'peter'],
    // Dice roguelike fields
    element: 'Death',
    preferredDice: 8,
    weakToDice: [6],       // Earth beats Death
    resistantToDice: [10], // Death beats Fire
    baseHp: 120,
    baseDamage: 18,
    baseDefense: 14,
    baseSpeed: 6,
    level: 28,
    encounterWeight: 50,
    minFloor: 3,
    drops: [
      { item: 'gold-sword', rate: 15, rarity: 'Rare' },
      { item: 'shadow-essence', rate: 35, rarity: 'Uncommon' },
      { item: 'bone-dust', rate: 60, rarity: 'Common' },
      { item: 'iron-key', rate: 20, rarity: 'Common' },
    ],
  },
  {
    slug: 'steam-sentry',
    name: 'Steam Sentry',
    category: 'enemies',
    enemyType: 'Elite',
    rarity: 'Uncommon',
    luckyNumber: 2,
    domain: 'earth',
    image: '/assets/enemies-svg/steam-sentry.svg',
    description: 'This cylindrical automaton with glowing cyan visor transforms thermodynamics into weapon systems. Eight-foot brass constructs patrol with pressure cooker temperament, venting steam in Morse code profanity patterns while maintaining Victorian sensibilities.',
    seeAlso: ['earth', 'gear-construct', 'john'],
    // Dice roguelike fields
    element: 'Earth',
    preferredDice: 6,
    weakToDice: [4],       // Void beats Earth
    resistantToDice: [8],  // Earth beats Death
    baseHp: 160,
    baseDamage: 16,
    baseDefense: 18,
    baseSpeed: 4,
    level: 18,
    encounterWeight: 50,
    minFloor: 3,
  },
  {
    slug: 'succubus',
    name: 'Succubus',
    category: 'enemies',
    enemyType: 'Elite',
    rarity: 'Rare',
    luckyNumber: 4,
    domain: 'infernus',
    image: '/assets/enemies-svg/succubus.svg',
    description: 'This turquoise-winged demon with golden accents weaponizes emotional vulnerabilities through psychological warfare. Operating beyond temptress tropes, she performs hostile takeovers of decision-making faculties. One variant discovered healing\'s superiority, becoming wanderer Stitch Up Girl.',
    seeAlso: ['infernus', 'stitch-up-girl', 'mephisto', 'robert'],
    // Dice roguelike fields
    element: 'Fire',
    preferredDice: 10,
    weakToDice: [8],       // Death beats Fire
    resistantToDice: [12], // Fire beats Ice
    baseHp: 95,
    baseDamage: 24,
    baseDefense: 6,
    baseSpeed: 11,
    level: 26,
    encounterWeight: 25,
    minFloor: 4,
  },
  {
    slug: 'time-wraith',
    name: 'Time Wraith',
    category: 'enemies',
    enemyType: 'Elite',
    rarity: 'Rare',
    luckyNumber: 0,
    domain: 'null-providence',
    image: '/assets/enemies-svg/time-wraith.svg',
    description: 'This blue-hooded reaper serves as cosmic janitor for temporal paradoxes. These spectral entities exist at death-time intersection, cleaning causality messes by erasing cause, effect, and witnesses. Dr. Voss variant questions existence while conducting experiments.',
    seeAlso: ['null-providence', 'dr-voss', 'time-elemental', 'the-one'],
    // Dice roguelike fields
    element: 'Void',
    preferredDice: 4,
    weakToDice: [20],      // Wind beats Void
    resistantToDice: [6],  // Void beats Earth
    baseHp: 85,
    baseDamage: 26,
    baseDefense: 4,
    baseSpeed: 14,
    level: 40,
    encounterWeight: 25,
    minFloor: 5,
  },
  {
    slug: 'wretched-beast',
    name: 'Wretched Beast',
    category: 'enemies',
    enemyType: 'Elite',
    rarity: 'Rare',
    luckyNumber: 6,
    domain: 'aberrant',
    image: '/assets/enemies-svg/wretched-beast.svg',
    description: 'This purple beast with green arms and flaming head proves evolution sometimes has nervous breakdowns. These creatures weaponized ugliness into combat mechanics—looking at them deals psychic damage. Reality flinches when they appear, embarrassed by what existence allows.',
    seeAlso: ['aberrant', 'abominable', 'jane'],
    // Dice roguelike fields
    element: 'Wind',
    preferredDice: 20,
    weakToDice: [12],      // Ice beats Wind
    resistantToDice: [4],  // Wind beats Void
    baseHp: 140,
    baseDamage: 28,
    baseDefense: 8,
    baseSpeed: 9,
    level: 42,
    encounterWeight: 25,
    minFloor: 6,
  },

  // ========================================
  // NORMAL ENEMIES
  // ========================================
  {
    slug: 'antworm',
    name: 'Antworm',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Common',
    domain: 'universal',
    image: '/assets/enemies-svg/antworm.svg',
    description: 'Segmented hybrid creatures that tunnel through domains with existential confusion. Their orange-red heads house compound eyes that see in too many directions, while blue-purple bodies writhe through self-made tunnel networks that spell out complaints in dead languages.',
    seeAlso: ['myconid', 'crab'],
    // Dice roguelike fields
    element: 'Neutral',
    baseHp: 35,
    baseDamage: 8,
    baseDefense: 3,
    baseSpeed: 6,
    level: 5,
    encounterWeight: 80,
    minFloor: 1,
  },
  {
    slug: 'bat',
    name: 'Bat',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Common',
    luckyNumber: 3,
    domain: 'shadow-keep',
    image: '/assets/enemies-svg/bat.svg',
    description: 'Tutorial enemy that introduces players to dying repeatedly. These purple-winged creatures flutter through every domain with erratic patterns designed to frustrate new players into understanding that death is essential, not optional.',
    seeAlso: ['shadow-keep', 'peter'],
    // Dice roguelike fields
    element: 'Death',
    preferredDice: 8,
    weakToDice: [6],       // Earth beats Death
    resistantToDice: [10], // Death beats Fire
    baseHp: 25,
    baseDamage: 6,
    baseDefense: 2,
    baseSpeed: 10,
    level: 1,
    encounterWeight: 80,
    minFloor: 1,
    drops: [
      { item: 'bone-dust', rate: 40, rarity: 'Common' },
      { item: 'blood-vial', rate: 25, rarity: 'Common' },
    ],
  },
  {
    slug: 'camel',
    name: 'Camel',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Uncommon',
    luckyNumber: 4,
    domain: 'infernus',
    image: '/assets/enemies-svg/camel.svg',
    description: 'Desert tanks that weaponize patience into spite. These tan ungulates patrol hot domains with humps full of condensed contempt, maintaining disturbing eye contact while plotting elaborate revenge for imagined slights.',
    seeAlso: ['infernus', 'fire-imp'],
    // Dice roguelike fields
    element: 'Fire',
    preferredDice: 10,
    weakToDice: [8],       // Death beats Fire
    resistantToDice: [12], // Fire beats Ice
    baseHp: 60,
    baseDamage: 10,
    baseDefense: 8,
    baseSpeed: 4,
    level: 22,
    encounterWeight: 50,
    minFloor: 2,
  },
  {
    slug: 'carniflower',
    name: 'Carniflower',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Uncommon',
    luckyNumber: 6,
    domain: 'aberrant',
    image: '/assets/enemies-svg/carniflower.svg',
    description: 'Carniflowers achieved dietary enlightenment through Jane\'s atmospheric experiments. These pink carnivorous plants photosynthesize revenge while breathing backwards. Six-petaled horror contains 666 tiny mouths seeking vegetable justice.',
    seeAlso: ['aberrant', 'twisted-sapling', 'jane'],
    // Dice roguelike fields
    element: 'Wind',
    preferredDice: 20,
    weakToDice: [12],      // Ice beats Wind
    resistantToDice: [4],  // Wind beats Void
    baseHp: 45,
    baseDamage: 14,
    baseDefense: 4,
    baseSpeed: 2,
    level: 36,
    encounterWeight: 50,
    minFloor: 3,
  },
  {
    slug: 'chicken',
    name: 'Chicken',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Common',
    luckyNumber: 6,
    domain: 'aberrant',
    image: '/assets/enemies-svg/chicken.svg',
    description: 'White-feathered barnyard terrors that scale aggression to player luck. These poultry enemies track every dice roll with mathematical precision, attacking harder when you roll poorly (sympathy rage) or excellently (jealousy fury).',
    seeAlso: ['aberrant', 'cow', 'jane'],
    // Dice roguelike fields
    element: 'Wind',
    preferredDice: 20,
    weakToDice: [12],      // Ice beats Wind
    resistantToDice: [4],  // Wind beats Void
    baseHp: 30,
    baseDamage: 10,
    baseDefense: 2,
    baseSpeed: 8,
    level: 35,
    encounterWeight: 80,
    minFloor: 1,
  },
  {
    slug: 'cow',
    name: 'Cow',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Common',
    luckyNumber: 1,
    domain: 'earth',
    image: '/assets/enemies-svg/cow.svg',
    description: 'Brown and white bovines that weaponize pastoral fury based on your dice rolls. These militarized livestock seek revenge for hamburgers consumed, with attack patterns scaling to both your luck and dietary choices.',
    seeAlso: ['earth', 'chicken', 'john'],
    // Dice roguelike fields
    element: 'Earth',
    preferredDice: 6,
    weakToDice: [4],       // Void beats Earth
    resistantToDice: [8],  // Earth beats Death
    baseHp: 50,
    baseDamage: 12,
    baseDefense: 5,
    baseSpeed: 4,
    level: 12,
    encounterWeight: 80,
    minFloor: 1,
  },
  {
    slug: 'crab',
    name: 'Crab',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Common',
    luckyNumber: 1,
    domain: 'earth',
    image: '/assets/enemies-svg/crab.svg',
    description: 'Cyan-shelled crustaceans that move exclusively sideways and calculate attacks based on dice probability. These mathematical warriors prefer rolls of 3 or 4, becoming exponentially more dangerous when dice match their lateral philosophy.',
    seeAlso: ['earth', 'myconid', 'john'],
    // Dice roguelike fields
    element: 'Earth',
    preferredDice: 6,
    weakToDice: [4],       // Void beats Earth
    resistantToDice: [8],  // Earth beats Death
    baseHp: 40,
    baseDamage: 8,
    baseDefense: 10,
    baseSpeed: 5,
    level: 10,
    encounterWeight: 80,
    minFloor: 1,
  },
  {
    slug: 'fire-imp',
    name: 'Fire Imp',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Common',
    luckyNumber: 4,
    domain: 'infernus',
    image: '/assets/enemies-svg/fire-imp.svg',
    description: 'Yellow-skinned warriors with flaming hair who patrol Infernus with military discipline. Despite their "imp" designation, these six-foot soldiers wield spears and shields with tactical precision, eternally frustrated by their cutesy classification.',
    seeAlso: ['infernus', 'lava-golem', 'robert'],
    // Dice roguelike fields
    element: 'Fire',
    preferredDice: 10,
    weakToDice: [8],       // Death beats Fire
    resistantToDice: [12], // Fire beats Ice
    baseHp: 45,
    baseDamage: 12,
    baseDefense: 5,
    baseSpeed: 7,
    level: 20,
    encounterWeight: 80,
    minFloor: 1,
    drops: [
      { item: 'infernal-salts', rate: 25, rarity: 'Uncommon' },
      { item: 'ember-fragment', rate: 35, rarity: 'Uncommon' },
      { item: 'ruby', rate: 10, rarity: 'Rare' },
      { item: 'infernal-key', rate: 5, rarity: 'Rare' },
    ],
  },
  {
    slug: 'gear-construct',
    name: 'Gear Construct',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Uncommon',
    luckyNumber: 6,
    domain: 'aberrant',
    image: '/assets/enemies-svg/gear-construct.svg',
    description: 'This purple-wrapped warrior topped with golden gears represents bureaucratic naming gone wrong. Despite the mechanical moniker, no actual gears exist—just ancient bandages and a crown that defies categorization. The confused mummy seeks pyramids in mechanical realms.',
    seeAlso: ['aberrant', 'steam-sentry', 'jane'],
    // Dice roguelike fields
    element: 'Wind',
    preferredDice: 20,
    weakToDice: [12],      // Ice beats Wind
    resistantToDice: [4],  // Wind beats Void
    baseHp: 55,
    baseDamage: 14,
    baseDefense: 8,
    baseSpeed: 5,
    level: 38,
    encounterWeight: 50,
    minFloor: 3,
  },
  {
    slug: 'ice-wraith',
    name: 'Ice Wraith',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Uncommon',
    luckyNumber: 5,
    domain: 'frost-reach',
    image: '/assets/enemies-svg/ice-wraith.svg',
    description: 'This pale blue ghost with glowing cyan eyes haunts frost-reach through bureaucratic assignment rather than elemental affinity. The spectral form lacks any actual ice powers, floating through frozen domains while questioning its job placement.',
    seeAlso: ['frost-reach', 'time-wraith', 'alice'],
    // Dice roguelike fields
    element: 'Ice',
    preferredDice: 12,
    weakToDice: [10],      // Fire beats Ice
    resistantToDice: [20], // Ice beats Wind
    baseHp: 40,
    baseDamage: 15,
    baseDefense: 3,
    baseSpeed: 9,
    level: 28,
    encounterWeight: 50,
    minFloor: 2,
  },
  {
    slug: 'myconid',
    name: 'Myconid',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Common',
    luckyNumber: 1,
    domain: 'earth',
    image: '/assets/enemies-svg/myconid.svg',
    description: 'This walking mushroom with white stem legs and red spotted cap represents actual fungal biology in combat form. Unlike other misnamed enemies, Myconids genuinely embody mushroom characteristics with bipedal locomotion added for mobility.',
    seeAlso: ['earth', 'spore-cloud', 'john'],
    // Dice roguelike fields
    element: 'Earth',
    preferredDice: 6,
    weakToDice: [4],       // Void beats Earth
    resistantToDice: [8],  // Earth beats Death
    baseHp: 35,
    baseDamage: 10,
    baseDefense: 4,
    baseSpeed: 5,
    level: 14,
    encounterWeight: 80,
    minFloor: 1,
  },
  {
    slug: 'parrot',
    name: 'Parrot',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Uncommon',
    domain: 'universal',
    image: '/assets/enemies-svg/parrot.svg',
    description: 'This red-headed avian with blue-teal wings evolved beyond simple mimicry into psychological warfare. These sound terrorists attack by repeating player failures in mocking tones, serving as both enemy and narrator of incompetence through selective audio playback.',
    seeAlso: ['bat', 'chicken'],
    // Dice roguelike fields
    element: 'Neutral',
    baseHp: 30,
    baseDamage: 12,
    baseDefense: 2,
    baseSpeed: 11,
    level: 15,
    encounterWeight: 50,
    minFloor: 2,
  },
  {
    slug: 'skeleton-archer',
    name: 'Skeleton Archer',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Common',
    luckyNumber: 3,
    domain: 'shadow-keep',
    image: '/assets/enemies-svg/skeleton-archer.svg',
    description: 'This skeletal marksman in golden armor with red bow masters ranged combat through post-mortem advantages. Death enhanced accuracy by removing breathing and blinking distractions. They patrol as bony turrets with supernatural precision.',
    seeAlso: ['shadow-keep', 'skeleton-knight', 'skeleton-barb', 'peter'],
    // Dice roguelike fields
    element: 'Death',
    preferredDice: 8,
    weakToDice: [6],       // Earth beats Death
    resistantToDice: [10], // Death beats Fire
    baseHp: 35,
    baseDamage: 14,
    baseDefense: 3,
    baseSpeed: 6,
    level: 26,
    encounterWeight: 80,
    minFloor: 1,
    drops: [
      { item: 'bone-dust', rate: 50, rarity: 'Common' },
      { item: 'shadow-bow', rate: 8, rarity: 'Rare' },
      { item: 'shadow-essence', rate: 20, rarity: 'Uncommon' },
    ],
  },
  {
    slug: 'skeleton-barb',
    name: 'Skeleton Barb',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Common',
    luckyNumber: 3,
    domain: 'shadow-keep',
    image: '/assets/enemies-svg/skeleton-barb.svg',
    description: 'This skeletal berserker in barbarian furs with blue sword and shield proves death cannot stop true rage. Without muscles to tire or blood to cool, they maintain perpetual fury. Removing flesh removed all limitations on violence potential.',
    seeAlso: ['shadow-keep', 'skeleton-knight', 'skeleton-archer', 'peter'],
    // Dice roguelike fields
    element: 'Death',
    preferredDice: 8,
    weakToDice: [6],       // Earth beats Death
    resistantToDice: [10], // Death beats Fire
    baseHp: 45,
    baseDamage: 16,
    baseDefense: 6,
    baseSpeed: 7,
    level: 28,
    encounterWeight: 80,
    minFloor: 2,
  },
  {
    slug: 'spore-cloud',
    name: 'Spore Cloud',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Common',
    luckyNumber: 6,
    domain: 'aberrant',
    image: '/assets/enemies-svg/spore-cloud.svg',
    description: 'This gray cloud with toxic green rain represents billions of microscopic entities forming collective consciousness. These atmospheric infections drift through domains as airborne plagues, transforming breathing into tactical suicide while weapons pass harmlessly through.',
    seeAlso: ['aberrant', 'myconid', 'jane'],
    // Dice roguelike fields
    element: 'Wind',
    preferredDice: 20,
    weakToDice: [12],      // Ice beats Wind
    resistantToDice: [4],  // Wind beats Void
    baseHp: 25,
    baseDamage: 8,
    baseDefense: 1,
    baseSpeed: 3,
    level: 36,
    encounterWeight: 80,
    minFloor: 1,
  },
  {
    slug: 'time-elemental',
    name: 'Time Elemental',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Uncommon',
    luckyNumber: 0,
    domain: 'null-providence',
    image: '/assets/enemies-svg/time-elemental.svg',
    description: 'This green humanoid warrior represents another cosmic naming failure. Despite "Time Elemental" designation, appears as standard fighter with brown armor and no temporal abilities. Living paradoxes exist across all moments while fighting conventionally.',
    seeAlso: ['null-providence', 'time-wraith', 'the-one'],
    // Dice roguelike fields
    element: 'Void',
    preferredDice: 4,
    weakToDice: [20],      // Wind beats Void
    resistantToDice: [6],  // Void beats Earth
    baseHp: 50,
    baseDamage: 14,
    baseDefense: 6,
    baseSpeed: 8,
    level: 38,
    encounterWeight: 50,
    minFloor: 3,
  },
  {
    slug: 'time-scavenger',
    name: 'Time Scavenger',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Uncommon',
    luckyNumber: 0,
    domain: 'null-providence',
    image: '/assets/enemies-svg/time-scavenger.svg',
    description: 'This blue-hooded thief with golden armor represents temporal kleptomania taken to cosmic extremes. These hunched creatures pickpocket moments from timelines, running through domains with twitchy energy while reaching through present to pilfer past.',
    seeAlso: ['null-providence', 'time-wraith', 'the-one'],
    // Dice roguelike fields
    element: 'Void',
    preferredDice: 4,
    weakToDice: [20],      // Wind beats Void
    resistantToDice: [6],  // Void beats Earth
    baseHp: 40,
    baseDamage: 12,
    baseDefense: 4,
    baseSpeed: 12,
    level: 36,
    encounterWeight: 50,
    minFloor: 2,
  },
  {
    slug: 'twisted-sapling',
    name: 'Twisted Sapling',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Common',
    luckyNumber: 6,
    domain: 'aberrant',
    image: '/assets/enemies-svg/twisted-sapling.svg',
    description: 'This brown sapling with bright green leaves represents botanical adolescence gone violent. These teenage trees channel growth hormones into combat, sprouting weapons instead of normal branches. Their twisted forms show nature\'s awkward phase armed with murderous intent.',
    seeAlso: ['aberrant', 'ancient-treant', 'vine-strangler', 'jane'],
    // Dice roguelike fields
    element: 'Wind',
    preferredDice: 20,
    weakToDice: [12],      // Ice beats Wind
    resistantToDice: [4],  // Wind beats Void
    baseHp: 40,
    baseDamage: 10,
    baseDefense: 5,
    baseSpeed: 4,
    level: 35,
    encounterWeight: 80,
    minFloor: 1,
  },
  {
    slug: 'vine-strangler',
    name: 'Vine Strangler',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Uncommon',
    luckyNumber: 6,
    domain: 'aberrant',
    image: '/assets/enemies-svg/vine-strangler.svg',
    description: 'This serpentine vine with green tendrils evolved consciousness purely for vengeance. These botanical constrictors view necks as unclaimed territory and throats as virgin trellis. They photosynthesize suffering while growing stronger on screams.',
    seeAlso: ['aberrant', 'twisted-sapling', 'ancient-treant', 'jane'],
    // Dice roguelike fields
    element: 'Wind',
    preferredDice: 20,
    weakToDice: [12],      // Ice beats Wind
    resistantToDice: [4],  // Wind beats Void
    baseHp: 50,
    baseDamage: 16,
    baseDefense: 3,
    baseSpeed: 6,
    level: 38,
    encounterWeight: 50,
    minFloor: 3,
  },
  {
    slug: 'void-spawn',
    name: 'Void Spawn',
    category: 'enemies',
    enemyType: 'Normal',
    rarity: 'Uncommon',
    luckyNumber: 0,
    domain: 'null-providence',
    image: '/assets/enemies-svg/void-spawn.svg',
    description: 'This purple orb with green energy represents aggressive absence incarnate. Void Spawn carries non-existence like disease, spreading reality deletion through everything touched. Health doesn\'t decrease—it forgets existing. Abilities don\'t fail—they cease having ever been.',
    seeAlso: ['null-providence', 'tentacle-beast', 'the-one'],
    // Dice roguelike fields
    element: 'Void',
    preferredDice: 4,
    weakToDice: [20],      // Wind beats Void
    resistantToDice: [6],  // Void beats Earth
    baseHp: 55,
    baseDamage: 18,
    baseDefense: 2,
    baseSpeed: 7,
    level: 40,
    encounterWeight: 50,
    minFloor: 3,
    drops: [
      { item: 'void-shard', rate: 40, rarity: 'Uncommon' },
      { item: 'void-salts', rate: 30, rarity: 'Rare' },
      { item: 'void-crystal', rate: 10, rarity: 'Epic' },
      { item: 'cursed-finger', rate: 5, rarity: 'Rare' },
    ],
  },
];
